...
if uploaded_file:
    ...
    if "Case reference" in df.columns:
        ...
        # Extract Emails
        st.subheader("Extracted Email Exchanges")
        email_texts = []
        for text in df[column_name].tolist():
            email_texts.extend(extract_emails(text))  # Extract emails from each entry

        if email_texts:
            for email in email_texts[:5]:  # Displaying only first 5 emails for brevity
                st.text_area("Email:", value=email, height=150)
        ...
        st.error("The 'Case reference' column is missing from the file. Please upload a valid dataset.") import streamlit as st
import pandas as pd
from sklearn.feature_extraction.text import TfidfVectorizer, CountVectorizer
from sklearn.decomposition import LatentDirichletAllocation
from collections import Counter
